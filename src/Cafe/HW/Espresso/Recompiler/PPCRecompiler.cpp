...
struct PPCInvalidationRange
{
	MPTR startAddress;
	uint32 size;
	// sequence number at the time this invalidation was recorded. Lets each concurrently-compiling
	// worker thread determine whether this invalidation happened during its own compile window,
	// without relying on the (now incorrect, see below) assumption that only one function is ever
	// being compiled at a time.
	uint64 sequence;

	PPCInvalidationRange(MPTR _startAddress, uint32 _size, uint64 _sequence) : startAddress(_startAddress), size(_size), sequence(_sequence) {};
};
...
struct
{
	...
	// monotonically increasing, bumped once per call to PPCRecompiler_invalidateRange (while recompilerSpinlock is held)
	uint64 invalidationSequenceCounter = 0;
	// sequence value captured by each worker thread when it starts compiling a function.
	// A multiset because several workers can be compiling at once (duplicates are expected).
	std::multiset<uint64> inFlightCompileStartSequence;
}PPCRecompilerState;
...
bool PPCRecompiler_makeRecompiledFunctionActive(uint32 initialEntryPoint, PPCFunctionBoundaryTracker::PPCRange_t& range, PPCRecFunction_t* ppcRecFunc, std::vector<std::pair<MPTR, uint32>>& entryPoints, uint64 compileStartSequence)
{
	// update jump table
	PPCRecompilerState.recompilerSpinlock.lock();
	...
	// IMPORTANT: with multiple recompiler worker threads compiling different functions at the same
	// time, PPCRecompilerState.invalidationRanges can contain entries that belong to another
	// worker's still in-flight compile window. We must only look at (and never blanket-clear) the
	// entries recorded after *this* job started, identified via compileStartSequence -- see
	// PPCRecompiler_recompileAtAddress. Entries from before our job started are irrelevant to us
	// (we started reading the game's code after they happened), and clearing the whole list here
	// would silently make another worker miss an invalidation and activate stale code.
	bool isInvalidated = false;
	for (auto& invRange : PPCRecompilerState.invalidationRanges)
	{
		if (invRange.sequence <= compileStartSequence)
			continue; // happened before our compile window started, not relevant to us
...
void PPCRecompiler_recompileAtAddress(uint32 address)
{
	...
	// also snapshot the current invalidation sequence number and register this job as "in-flight":
	// this tells PPCRecompiler_makeRecompiledFunctionActive() which invalidation events happened
	// during *this* job's compile window (see comments there), and tells
	// PPCRecompiler_invalidateRange() that it must not prune those events away while we're still
	// running, even if another worker thread finishes and prunes in the meantime.
	PPCRecompilerState.recompilerSpinlock.lock();

	std::set<uint32> entryAddresses;

	entryAddresses.emplace(address);
	uint64 myStartSequence = PPCRecompilerState.invalidationSequenceCounter;
	PPCRecompilerState.inFlightCompileStartSequence.emplace(myStartSequence);

	PPCRecompilerState.recompilerSpinlock.unlock();

	// deregister this job when we leave, regardless of which return path we take
	struct InFlightGuard
	{
		...
		{
			PPCRecompilerState.recompilerSpinlock.lock();
			auto it = PPCRecompilerState.inFlightCompileStartSequence.find(seq);
			if (it != PPCRecompilerState.inFlightCompileStartSequence.end())
				PPCRecompilerState.inFlightCompileStartSequence.erase(it);
			PPCRecompilerState.recompilerSpinlock.unlock();
		}
	} inFlightGuard{myStartSequence};
	...
	bool r = PPCRecompiler_makeRecompiledFunctionActive(address, range, func, functionEntryPoints, myStartSequence);
}

std::vector<std::thread> s_threadsRecompiler;
std::atomic_bool s_recompilerThreadStopSignal{false};

// Number of background worker threads used to JIT-compile PPC functions in parallel.
// A single compiler thread is a disproportionately large bottleneck on mobile SoCs, where
// per-core performance is much lower than desktop and stutter during loading screens / area
// transitions (when many functions are queued at once) is very noticeable.
// We deliberately stay conservative: we never want the recompiler pool to starve the 3 emulated
// PPC cores, the GPU submission thread or the audio thread of CPU time, so we reserve headroom
// instead of using all available cores.
static uint32 PPCRecompiler_GetWorkerThreadCount()
{
	// allow forcing a specific worker count for debugging/benchmarking via command line
	sint32 forcedCount = LaunchSettings::GetPPCRecCompilerThreadCount();
	if (forcedCount > 0)
		return (uint32)std::min(forcedCount, 8);

	uint32 hwThreads = std::thread::hardware_concurrency();
	if (hwThreads == 0)
		hwThreads = 4; // conservative fallback if the OS can't report core count

	uint32 workerCount;
	if (hwThreads <= 4)
		workerCount = 1;      // low-end / older devices: keep legacy single-thread behavior
	else if (hwThreads <= 6)
		workerCount = 2;
	else if (hwThreads <= 8)
		workerCount = 3;
	else
		workerCount = 4;      // high-core-count devices, still leaves plenty of room

	return workerCount;
}

void PPCRecompiler_thread(uint32 workerIndex)
{
	SetThreadName(fmt::format("PPCRecompiler_{}", workerIndex).c_str());
	// Pin to the fastest core cluster on heterogeneous (big.LITTLE) SoCs so
	// this worker doesn't get scheduled onto a "little" efficiency core -
	// a no-op on homogeneous CPUs. See Common/CpuAffinity.h for details.
	CpuAffinity::PinCurrentThread(CpuAffinity::ThreadRole::JitCompiler);
#if PPCREC_FORCE_SYNCHRONOUS_COMPILATION
	return;
#endif
...
void PPCRecompiler_invalidateRange(uint32 startAddr, uint32 endAddr)
{
	...
	PPCRecompilerState.invalidationSequenceCounter++;
	PPCRecompilerState.invalidationRanges.emplace_back(startAddr, endAddr - startAddr, PPCRecompilerState.invalidationSequenceCounter);

	// prune invalidation entries that no currently in-flight compile job can still need.
	// A job only needs to see entries recorded after it started, so once every job that started
	// before a given entry has finished, that entry can never be relevant again.
	uint64 minInFlightStart = PPCRecompilerState.inFlightCompileStartSequence.empty() ? PPCRecompilerState.invalidationSequenceCounter : *PPCRecompilerState.inFlightCompileStartSequence.begin();
	std::erase_if(PPCRecompilerState.invalidationRanges, [minInFlightStart](const PPCInvalidationRange& r) { return r.sequence <= minInFlightStart; });
...
void PPCRecompiler_init()
{
	...
	// launch recompilation worker thread pool
    s_recompilerThreadStopSignal = false;
    uint32 workerCount = PPCRecompiler_GetWorkerThreadCount();
    cemuLog_log(LogType::Force, "PPCRecompiler: starting {} worker thread(s)", workerCount);
    s_threadsRecompiler.reserve(workerCount);
    for (uint32 i = 0; i < workerCount; i++)
        s_threadsRecompiler.emplace_back(PPCRecompiler_thread, i);
}

void PPCRecompiler_Shutdown()
{
    // shut down recompiler worker threads
    s_recompilerThreadStopSignal = true;
    for (auto& t : s_threadsRecompiler)
    {
        if (t.joinable())
            t.join();
    }
    s_threadsRecompiler.clear();
    // clean up queues
    while(!PPCRecompilerState.targetQueue.empty())
        PPCRecompilerState.targetQueue.pop();
    PPCRecompilerState.invalidationRanges.clear();
    PPCRecompilerState.invalidationSequenceCounter = 0;
    // sanity check: every worker thread has been joined above, so no compile job should still be
    // registered as in-flight at this point
    cemu_assert_debug(PPCRecompilerState.inFlightCompileStartSequence.empty());
    PPCRecompilerState.inFlightCompileStartSequence.clear();
    // clean range store
