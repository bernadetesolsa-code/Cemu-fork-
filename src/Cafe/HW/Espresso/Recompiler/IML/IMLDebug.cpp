...
std::string IMLDebug_GetRegName(IMLReg r)
{
	...
	case IMLRegFormat::F64:
		regName.append("fd");
		break;
	case IMLRegFormat::F64X2:
		regName.append("fq"); // reserved format, not yet allocated anywhere - see IMLRegFormat::F64X2 comment
		break;
	...
	if (r.GetViewOffset() != 0)
		regName.append(fmt::format(".{}", r.GetViewOffset()));
	return regName;
}
