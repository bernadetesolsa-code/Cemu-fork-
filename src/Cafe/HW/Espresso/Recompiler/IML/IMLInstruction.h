...
enum class IMLRegFormat : uint8
{
	...
	F64,
	F32,
	// 128bit vector register holding two packed doubles (PowerPC paired-single, ps0 in element 0 / ps1 in element 1).
	// Reserved for future use: as of fase 6/7 the register allocator does not allocate physical registers for this
	// format yet (perTypePhysPool for it is always empty), and no IML generator emits registers with this base
	// format. It exists so IMLReg can already express "F64 view at element offset N of an F64X2 base register"
	// via the viewOffset field below, which is the representation a future paired-single vectorization pass needs.
	//
	// IMPORTANT hazard for whoever wires this in on AArch64: a scalar FP instruction (e.g. `fadd d0, d1, d2`) zeroes
	// the upper 64 bits of the destination V register per the AArch64 ISA. That means ps0 and ps1 cannot simply be
	// allocated into lane 0 / lane 1 of the same physical V register while other code still treats them as
	// independent scalar F64 registers - any scalar op on one lane silently destroys the other. A correct
	// implementation must track, per live range, whether a paired register is currently "resident as a vector" or
	// "split into independent scalars", and materialize/rejoin at the transition points (register allocator work,
	// not just codegen). See relatorio-fase7-fpu-vetorial.md for details.
	F64X2,
	TYPE_COUNT,
};
...
public:
	...
	// baseRegFormat: format of the underlying physical register (e.g. F64X2 for a 128bit vector register)
	// regFormat: format this particular view of the register is accessed as (e.g. F64 for one lane of a paired vector register)
	// viewOffset: element offset (in units of regFormat's size) of this view within the base register. Always 0 unless baseRegFormat != regFormat.
	IMLReg(IMLRegFormat baseRegFormat, IMLRegFormat regFormat, uint8 viewOffset, IMLRegID regId)
	{
		cemu_assert_debug(viewOffset < 32);
		m_raw = 0;
		m_raw |= ((uint8)baseRegFormat << 28);
		m_raw |= ((uint8)regFormat << 24);
		m_raw |= ((uint32)(viewOffset & 0x1F) << 19);
		m_raw |= (uint32)regId;
	}
	...
	// element offset (in units of GetRegFormat()'s size) of this view within the base register.
	// 0 for all registers except sub-views of a wider base register (e.g. F64 view of an F64X2 pair).
	uint8 GetViewOffset() const
	{
		return (uint8)((m_raw >> 19) & 0x1F);
	}
