...
void MMURange::mapMem()
{
	cemu_assert_debug(!m_isMapped);
		if (memory_base == nullptr || MemMapper::AllocateMemory(memory_base + baseAddress, size, MemMapper::PAGE_PERMISSION::P_RW, true) == nullptr)
		{
			cemuLog_log(LogType::Force, "ERROR: Unable to allocate {} memory at {:08x}", name, baseAddress);
			// We mark it as NOT mapped and continue, hoping the emulated title doesn't access it immediately
			m_isMapped = false;
			return;
		}
	m_isMapped = true;
...
void memory_init()
{
	...
	if(!memory_base)
		{
			// Try to reserve 4GB at a high address first, then fallback to any available space
			// On Android/Linux, some address ranges might be restricted.
			memory_base = (uint8*)MemMapper::ReserveMemory((void*)0x2000000000, (size_t)0x100000000, MemMapper::PAGE_PERMISSION::P_RW);
			if (!memory_base)
		...
		if( !memory_base )
		{
			cemuLog_log(LogType::Force, "CRITICAL ERROR: Unable to reserve 4GB of memory space for Wii U emulation.");
			// Instead of immediate exit, we show dialog and let the app handle the failure more gracefully if possible
