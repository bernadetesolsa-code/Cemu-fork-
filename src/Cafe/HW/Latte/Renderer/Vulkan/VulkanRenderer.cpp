...
void VulkanRenderer::DetermineVendor()
{
	...
	case 0x106B:
		...
		break;
	case 0x13B5:
		// ARM Mali - tile-based deferred renderer (TBDR), used by most Android SoCs (Exynos, Dimensity, some Snapdragon)
		m_vendor = GfxVendor::Mali;
		break;
	}
...
VulkanRenderer::VulkanRenderer()
{
	...
	// extension info
	cemuLog_log(LogType::Force, "VK_KHR_dynamic_rendering: {}", m_featureControl.deviceExtensions.dynamic_rendering?"supported and enabled (Mali)":"not enabled");
...
bool VulkanRenderer::CheckDeviceExtensionSupport(const VkPhysicalDevice device, FeatureControl& info)
{
	...
	// dynamic rendering is disabled by default (driver implementations were very unoptimized as of Feb 2022 on desktop GPUs)
	// however on Mali (tile-based deferred renderer) it avoids the cost of legacy VkRenderPass/VkFramebuffer object churn
	// which is expensive specifically when a render pass has to be ended+restarted mid-frame (e.g. shadow maps sampled
	// right after being rendered to, see CachedFBOVk::CheckForCollision / vk_accurate_barriers)
	bool isMaliGpu = false;
	{
		VkPhysicalDeviceProperties maliCheckProps{};
		vkGetPhysicalDeviceProperties(device, &maliCheckProps);
		isMaliGpu = (maliCheckProps.vendorID == 0x13B5);
	}
	info.deviceExtensions.dynamic_rendering = isMaliGpu && isExtensionAvailable(VK_KHR_DYNAMIC_RENDERING_EXTENSION_NAME);
	info.deviceExtensions.depth_clip_enable = isExtensionAvailable(VK_EXT_DEPTH_CLIP_ENABLE_EXTENSION_NAME);
