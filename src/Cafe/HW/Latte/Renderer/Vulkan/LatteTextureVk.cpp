	...
	: LatteTexture(dim, physAddress, physMipAddress, format, width, height, depth, pitch, mipLevels, swizzle, tileMode, isDepth), m_vkr(vkRenderer)
{
	...
	// Validate against hardware limits before creating image
	// On mobile devices, exceeding maxImageDimension2D is a common cause of crashes when using high-res graphic packs
	VkPhysicalDeviceProperties properties;
	vkGetPhysicalDeviceProperties(m_vkr->GetPhysicalDevice(), &properties);
	
	bool dimensionsValid = true;
	if (imageInfo.imageType == VK_IMAGE_TYPE_1D && imageInfo.extent.width > properties.limits.maxImageDimension1D) dimensionsValid = false;
	if (imageInfo.imageType == VK_IMAGE_TYPE_2D && (imageInfo.extent.width > properties.limits.maxImageDimension2D || imageInfo.extent.height > properties.limits.maxImageDimension2D)) dimensionsValid = false;
	if (imageInfo.imageType == VK_IMAGE_TYPE_3D && (imageInfo.extent.width > properties.limits.maxImageDimension3D || imageInfo.extent.height > properties.limits.maxImageDimension3D || imageInfo.extent.depth > properties.limits.maxImageDimension3D)) dimensionsValid = false;

	if (!dimensionsValid)
	{
		cemuLog_log(LogType::Force, "GraphicPack: Texture dimensions ({}x{}x{}) exceed hardware limits (max 2D: {}). Resolution pack might be too high for this device.", 
			imageInfo.extent.width, imageInfo.extent.height, imageInfo.extent.depth, properties.limits.maxImageDimension2D);
		
		// Fallback to original dimensions if possible, or cap at hardware limits
		imageInfo.extent.width = std::min(imageInfo.extent.width, properties.limits.maxImageDimension2D);
		imageInfo.extent.height = std::min(imageInfo.extent.height, properties.limits.maxImageDimension2D);
	}

	if (vkCreateImage(m_vkr->GetLogicalDevice(), &imageInfo, nullptr, &vkObjTex->m_image) != VK_SUCCESS)
	{
		cemuLog_log(LogType::Force, "Failed to create texture image ({}x{} fmt:{:x})", imageInfo.extent.width, imageInfo.extent.height, (uint32)imageInfo.format);
		m_vkr->UnrecoverableError("Failed to create texture image. This usually happens when a Graphic Pack resolution is too high for the GPU memory or hardware limits.");
	}
