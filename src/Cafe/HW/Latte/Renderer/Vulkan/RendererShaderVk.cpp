...
public:
	...
	void CompilerThreadFunc()
	{
		...
		while (m_threadsActive.load(std::memory_order::relaxed))
		{
			...
			s_compilationQueueMutex.unlock();
			// compile
			// Safety net: this runs on a background thread. An uncaught exception here
			// would call std::terminate() and kill the whole application instantly with
			// no error message. Catch anything unexpected, log it, and mark the shader
			...
			{
				cemuLog_log(LogType::Force, "Shader compilation thread caught exception: {}", e.what());
				job->m_hasCompilationError = true;
			}
			catch (...)
			{
				cemuLog_log(LogType::Force, "Shader compilation thread caught unknown exception");
				job->m_hasCompilationError = true;
			}
...
void RendererShaderVk::CreateVkShaderModule(std::span<uint32> spirvBuffer)
{
	...
	{
		// Do not throw here: this can run on the async shader compiler thread, and an
		// uncaught exception on a worker thread terminates the entire process immediately
		// (no error dialog, app just closes). Instead, log the failure and leave the
		// shader module as VK_NULL_HANDLE so the caller can detect and skip it gracefully.
		cemuLog_log(LogType::Force, "Vulkan: Failed to create shader module for {:016x}_{:016x} (VkResult {})", m_baseHash, m_auxHash, (int32_t)result);
...
void RendererShaderVk::PreponeCompilation(bool isRenderThread)
{
	...
	else
	{
		...
		// Same safety net as the async path: never let an exception escape and crash the app.
		try
		{
		...
		{
			cemuLog_log(LogType::Force, "Shader compilation (sync) caught exception: {}", e.what());
			m_hasCompilationError = true;
		}
		catch (...)
		{
			cemuLog_log(LogType::Force, "Shader compilation (sync) caught unknown exception");
			m_hasCompilationError = true;
		}
