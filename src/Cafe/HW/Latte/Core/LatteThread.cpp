...
int Latte_ThreadEntry()
{
	...
	// GPU command submission is latency-critical and, on Android, competes
	// directly with the 3 emulated PPC cores and the JIT worker pool for the
	// SoC's fast cluster. Pin it there too so a "little" core never becomes
	// the frame-pacing bottleneck. No-op on homogeneous CPUs.
	CpuAffinity::PinCurrentThread(CpuAffinity::ThreadRole::GpuSubmission);
	sint32 w,h;
	WindowSystem::GetWindowPhysSize(w,h);

	// On Android, g_renderer is constructed earlier (JNI initializeRenderer) inside its
	// own try/catch. If Vulkan instance/device creation fails there (unsupported GPU,
	// missing extension, driver quirk), that failure is only logged - g_renderer is left
	// null - and emulation is still allowed to start. Dereferencing a null g_renderer here
	// is a native segfault, not a C++ exception, so it is NOT caught by the try/catch below
	// and kills the app instantly with no error message. This matches games crashing right
	// as they try to start, regardless of which game or which shader. Guard against it here.
	if (!g_renderer)
	{
		WindowSystem::ShowErrorDialog("The graphics renderer could not be initialized. Your device's GPU driver may not support the required Vulkan features.", "Graphics initialization error");
		return 0;
	}

	// renderer
	// Initialize() and InitializeStatic() can fail (e.g. a built-in blit/scaling shader
	// fails to compile on a particular GPU driver). Previously an exception here was
	// uncaught on this thread, which calls std::terminate() and kills the whole app
	// instantly right as emulation starts - no error message, just closes. Catch it,
	// show the user an actual error, and stop this thread cleanly instead.
	try
	{
	...
	{
		WindowSystem::ShowErrorDialog(fmt::format("Failed to initialize the graphics renderer:\n{}", ex.what()), "Graphics initialization error");
		return 0;
	}
	catch (...)
	{
		WindowSystem::ShowErrorDialog("Failed to initialize the graphics renderer due to an unknown error.", "Graphics initialization error");
		return 0;
	}
