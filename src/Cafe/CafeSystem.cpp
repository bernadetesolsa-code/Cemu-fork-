...
void cemu_initForGame()
{
	...
	cemuLog_createLogFile(false);
	// reset any custom vsync frequency and CPU timer speed scaling left over from a previously
	// running title. without this, launching game B (no graphic pack timing fix, "Limit to 30 FPS"
	// / "Emulation Speed" off) right after game A (which had custom timing active) would incorrectly
	// inherit game A's timing, since the process/state isn't restarted between titles on Android.
	LatteTiming_disableCustomVsyncFrequency();
	ActiveSettings::SetGameSpeedPercent(100);
	...
	debugger_handleEntryBreakpoint(_entryPoint);
		...
		{
			cemuLog_log(LogType::Force, "Crash averted: Failed to activate graphic packs: {}", ex.what());
		}
		catch (...)
		{
			cemuLog_log(LogType::Force, "Crash averted: Unknown error while activating graphic packs");
		}

	// --- user-requested timing overrides (Settings -> Graphics) ---
	// precedence, highest to lowest:
	//   1) a graphic pack's own curated custom vsync frequency (community-tested per-game fix)
	//   2) "Limit to 30 FPS"
	//   3) "Emulation Speed" (100-200%)
	// only one of 2/3 is ever applied, and neither ever overrides a graphic pack's own timing fix.
		sint32 existingCustomVsyncFreq = 0;
		bool graphicPackOwnsTiming = LatteTiming_getCustomVsyncFrequency(existingCustomVsyncFreq);
		if (GetConfig().limit_to_half_fps)
		{
			// Force 30 FPS if requested by user, unless a graphic pack has a specific timing fix.
			// Some graphic packs might set a frequency but we still want to respect the user's 30 FPS limit 
			// if it's a generic resolution pack. We only skip if it's a "Timing" related pack.
			if (!graphicPackOwnsTiming || existingCustomVsyncFreq > 30)
			{
				cemuLog_log(LogType::Force, "Limit to 30 FPS is enabled (Previous freq: {}) - setting vsync frequency to 30", graphicPackOwnsTiming ? std::to_string(existingCustomVsyncFreq) : "default");
				LatteTiming_setCustomVsyncFrequency(30);
				// We also need to ensure the game logic speed is adjusted if the game was targeting 60
				ActiveSettings::SetGameSpeedPercent(100); 
			}
			else
			{
				cemuLog_log(LogType::Force, "Limit to 30 FPS is enabled but a graphic pack already sets a lower or equal custom vsync frequency ({})", existingCustomVsyncFreq);
			}
		}
	else
	{
		// "Emulation Speed" - a genuine speed change (not a rendering trick): it drives the same
		// vsync-frequency mechanism graphic packs use, plus scales the PPC CPU timer by the same
		// factor so the console's own notion of elapsed time speeds up/slows down consistently.
		// Super Mario 3D World's main loop ticks one game-logic step per vsync signal, so raising
		// the vsync rate above ~60Hz genuinely makes the game play faster in real time, the same
		// way lowering it to 30Hz genuinely made it play at half speed.
		uint32 speedPercent = GetConfig().emulation_speed_percent.GetValue();
		if (speedPercent != 100)
		{
			if (!graphicPackOwnsTiming)
			{
				// scale from the console's native ~59.94Hz vsync rate (rounded, not truncated,
				// so e.g. 200% lands on 120 rather than being truncated down to 119)
				sint32 targetFreq = (sint32)((5994ull * (uint64)speedPercent + 5000ull) / 10000ull);
				if (targetFreq < 1)
					targetFreq = 1;
				if (targetFreq > 240) // sanity ceiling - well above what's practically usable
					targetFreq = 240;
				cemuLog_log(LogType::Force, "Emulation Speed is set to {}% - setting vsync frequency to {}", speedPercent, targetFreq);
				LatteTiming_setCustomVsyncFrequency(targetFreq);
				ActiveSettings::SetGameSpeedPercent(speedPercent);
			}
			else
			{
				cemuLog_log(LogType::Force, "Emulation Speed is set to {}% but a graphic pack already sets a custom vsync frequency ({}) - ignoring user setting", speedPercent, existingCustomVsyncFreq);
			}
		}
	}
...
namespace CafeSystem
{
	...
	PREPARE_STATUS_CODE PrepareForegroundTitle(TitleId titleId)
	{
		...
		if (r != PREPARE_STATUS_CODE::SUCCESS) {
			cemuLog_log(LogType::Force, "Failed to load/mount title {:016x}", titleId);
			return r;
		}
		
		// Ensure save directories exist before launching
		try {
			const auto mlc = ActiveSettings::GetMlcPath();
			fs::create_directories(mlc / "usr/save");
		} catch (...) {}
		gameProfile_load();
	...
	{
		// This thread handles game/title loading and starts the emulated CPU scheduler - it runs
		// before any GPU/renderer code is touched at all. An uncaught exception here (e.g. a
		// corrupted or unexpected file inside a module a game relies on) previously killed the
		// whole process via std::terminate(), same as the GPU-side bugs already fixed, except
		// this one is completely GPU-independent - it would crash identically regardless of
		// Adreno, Mali, or any other vendor, since it happens before any graphics work starts.
		try
		{
			for(auto& module : s_iosuModules)
				module->TitleStart();
			...
			// The scheduler returned on its own, meaning the title exited normally (e.g. the game
			// requested "Return to Wii U Menu", or simply ran to completion). Nothing else observes
			// this thread finishing, so without notifying the platform layer here, the game is over
			// but nobody outside this thread knows it - on Android that left EmulationActivity stuck
			// on the last rendered frame forever, with no way to get back.
			sSystemRunning = false;
			WindowSystem::NotifyGameExited();
		}
		catch (const std::exception& ex)
		{
			cemuLog_log(LogType::Force, "Fatal error while starting title: {}", ex.what());
			WindowSystem::ShowErrorDialog(fmt::format("Failed to start the title:\n{}", ex.what()), "Title launch error");
			sSystemRunning = false;
			WindowSystem::NotifyGameExited();
		}
		catch (...)
		{
			cemuLog_log(LogType::Force, "Fatal unknown error while starting title");
			WindowSystem::ShowErrorDialog("Failed to start the title due to an unknown error.", "Title launch error");
			sSystemRunning = false;
			WindowSystem::NotifyGameExited();
		}
	}
