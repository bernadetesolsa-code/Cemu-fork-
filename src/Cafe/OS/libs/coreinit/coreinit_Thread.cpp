...
namespace coreinit
{
	...
	void OSSchedulerCoreEmulationThread(void* _assignedCoreIndex)
	{
		...
		// Emulated PPC cores are the hottest threads in the process. On
		// heterogeneous (big.LITTLE) Android SoCs, pin them to the fastest
		// core cluster so the scheduler can't quietly demote one onto a
		// "little" efficiency core. No-op on homogeneous CPUs.
		CpuAffinity::PinCurrentThread(CpuAffinity::ThreadRole::EmulatedCpuCore);
