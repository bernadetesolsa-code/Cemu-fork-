...
bool LaunchSettings::HandleCommandline(const std::vector<std::wstring>& args)
{
	...
	hidden.add_options()
		...
		("ppcrec-threads", po::value<sint32>(), "Number of parallel PPC recompiler worker threads. 0 (default) = auto-detect from core count");
	...
	try
	{
		...
		if (vm.count("ppcrec-threads"))
		{
			s_ppcRec_compilerThreadCount = vm["ppcrec-threads"].as<sint32>();
			cemuLog_log(LogType::Force, "PPCRec worker thread count forced to {}", s_ppcRec_compilerThreadCount);
		}
