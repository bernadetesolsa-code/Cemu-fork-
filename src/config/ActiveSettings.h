...
public:
  ...
  static void SetInternalDir(const fs::path& internalDirPath);
	...
	// game speed (percent of native speed, 100 = normal). Distinct from GetTimerShiftFactor(),
	// which is a power-of-two debug-only speed used by the desktop "Timer Speed" menu.
	// This one is applied multiplicatively on top of it, and is meant for the real
	// "Emulation Speed" user-facing option (any percentage, e.g. 150 for 1.5x).
	[[nodiscard]] static uint32 GetGameSpeedPercent();

	static void SetGameSpeedPercent(uint32 speedPercent);
...
private:
	...
	inline static uint32 s_game_speed_percent = 100; // 100 = normal speed, e.g. 150 = 1.5x
