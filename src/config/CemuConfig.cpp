...
XMLConfigParser CemuConfig::Load(XMLConfigParser& parser)
{
	...
	limit_to_half_fps = graphic.get("LimitToHalfFps", false);
	emulation_speed_percent = graphic.get("EmulationSpeedPercent", 100u);
#if ENABLE_METAL
...
XMLConfigParser CemuConfig::Save(XMLConfigParser& parser)
{
	...
	graphic.set("LimitToHalfFps", limit_to_half_fps.GetValue());
	graphic.set("EmulationSpeedPercent", emulation_speed_percent.GetValue());
