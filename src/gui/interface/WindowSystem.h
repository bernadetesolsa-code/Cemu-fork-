...
namespace WindowSystem
{
	...
	// Lets a platform layer (e.g. the Android JNI glue, which this header's implementer cannot
	// depend on directly without creating a circular library dependency) hook into
	// ShowErrorDialog. Needed because on Android, fatal errors can happen on native background
	// threads (like the GPU/LatteThread) well after the UI thinks emulation started fine, with
	// nothing otherwise around to surface the error to the user.
	using ErrorDialogCallback = std::function<void(std::string_view message, std::string_view title)>;
	void SetErrorDialogCallback(ErrorDialogCallback callback);

	// Lets a platform layer (e.g. the Android JNI glue) hook into NotifyGameExited(). Needed
	// because on Android, the title-launch thread runs independently of the UI: when it finishes
	// (the title exited on its own, e.g. "Return to Wii U Menu", or the PPC scheduler simply
	// returned), nothing else automatically tells the Java/Kotlin side the game is over, so the
	// UI has no way to know it should leave the emulation screen.
	using GameExitedCallback = std::function<void()>;
	void SetGameExitedCallback(GameExitedCallback callback);
